...
<html>
    ...
    <body>
        <form class="form-group" method="post" action="saveUser?sessionId=<%=sessionId%>&key=<%=key%>&action=miperfil&id=<%=id%>" id="fo3">
  ...
  // Enviamos el formulario usando AJAX
        $.ajax({
            ...
            success: function(data) {
                var v = JSON.parse(data);
                if(v.error!=""){
                    loadContend("Customer Datos","contend/config/miperfil.jsp?id="+v.id+"");
                }
                else{
